...
def get_data_info(folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    X_path, Y = [], []
    X_len = []
    poses_df = pd.read_csv(par.pose_file,
                           index_col='Filename')  # columns: Filename,TrajectoryId,Timestamp,Easting,Northing,Height,Roll,Pitch,Yaw
    for folder in folder_list:
        ...
        if seq_len_range[0] == seq_len_range[1]:
            ...
            for st in start_frames:
                ...
                y_segs = [get_pose(x_seg, poses_df) for x_seg in
                          x_segs]  # [poses[i:i+seq_len] for i in range(st, n_frames, jump)]
                Y += y_segs
                X_path += x_segs
...
class ImageSequenceDataset(Dataset):
    def __init__(self, info_dataframe, resize_mode='crop', new_size=None, img_mean=None, img_std=(1, 1, 1),
                 minus_point_5=False, test_mode=False):
        # Transforms
        transform_ops = []
        if resize_mode == 'crop':
