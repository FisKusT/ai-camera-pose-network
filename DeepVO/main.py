...
wandb.init(project="camera-pose-estimation",
           config=par)
...
if os.path.isfile(par.train_data_info_path) and os.path.isfile(par.valid_data_info_path):
    ...
    if par.partition != None:
        ...
        train_df, valid_df = get_partition_data_info(partition, par.train_video, par.seq_len, overlap=6,
                                                     sample_times=par.sample_times, shuffle=True, sort=True)
    else:
        train_df = get_data_info(folder_list=par.train_video, seq_len_range=par.seq_len, overlap=6,
                                 sample_times=par.sample_times)
        valid_df = get_data_info(folder_list=par.valid_video, seq_len_range=par.seq_len, overlap=6,
                                 sample_times=par.sample_times)
    # save the data info
...
# Model
M_deepvo = DeepVO(par.img_h, par.img_w, par.batch_norm, angle_loss_weight=par.angle_loss_weight,
                  translation_loss_weight=par.translation_loss_weight, height_loss_weight=par.height_loss_weight)
use_cuda = torch.cuda.is_available()
if use_cuda:
    print('CUDA used.')
    M_deepvo = M_deepvo.cuda(device=par.device)
...
if par.optim['opt'] == 'Adam':
    optimizer = torch.optim.Adam(M_deepvo.parameters(), lr=par.optim['lr'], betas=(0.9, 0.999))
...
for ep in range(par.epochs):
    ...
    for _, t_x, t_y in tqdm(train_dl):
        batches += 1
        if use_cuda:
            t_x = t_x.cuda(non_blocking=par.pin_mem, device=par.device).float()
            t_y = t_y.cuda(non_blocking=par.pin_mem, device=par.device).float()
        ls = M_deepvo.step(t_x, t_y, optimizer).data.cpu().numpy()
        if batches % par.log_interval == 0:
            wandb.log({'loss': float(ls)}, step=batches)
    ...
    # save model
    if ep % par.save_interval == 0:
        torch.save(M_deepvo,
                   f"{par.save_model_path}_ep{ep}_angle{int(par.angle_loss_weight)}_trans{int(par.translation_loss_weight)}_height{int(par.height_loss_weight)}.pth")

    if ep % par.valid_interval == (par.valid_interval - 1):
        # Validation
        st_t = time.time()
        M_deepvo.eval()
        loss_mean_valid = 0
        ...
        for _, v_x, v_y in valid_dl:
            if use_cuda:
                v_x = v_x.cuda(non_blocking=par.pin_mem, device=par.device)
                v_y = v_y.cuda(non_blocking=par.pin_mem, device=par.device)
